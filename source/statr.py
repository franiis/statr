...
_statr = ctypes.CDLL('./build/statr_lib.so')
...
_statr.runif.restype = ctypes.c_double
...
def runif(n, min=0.0, max=1.0):
    ...
    result = float(_statr.runif(res, len(res), cmin, len(min), cmax, len(max)))
    print("runif method result: " + str(result))
    return list(res)
